...
try:
    ans[n] = round(matrix[n][n + 1] / matrix[n][n], NUMBER_OF_DECIMAL_PLACES)  # Находим последний корень
...
for i in range(n - 1, 0, -1):
    ...
    ans[i] = round((matrix[i][n + 1] - amount) / matrix[i][i], NUMBER_OF_DECIMAL_PLACES)
