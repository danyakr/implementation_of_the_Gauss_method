...
def print_update_matrix2(matrix):
    """
    Prints the matrix with aligned spacing for a column with customized 1-based indexing.

    Calculates the maximum length of elements in the column to align the elements properly,
    then iterates through the matrix and prints each element with the appropriate spacing.

    Args:
    matrix: MyList, the matrix to be printed with customized indexing starting from 1
    """
    lens = MyList([0] * len(matrix[1]))
    for j in range(1, len(matrix[1]) + 1):
        for i in range(1, len(matrix) + 1):
            if len(str(matrix[i][j])) > lens[j]:
                lens[j] = len(str(matrix[i][j]))
    ...
    for i in range(1, len(matrix) + 1):
        for j in range(1, len(matrix[i]) + 1):
            if j == len(matrix) + 1:
                ...
                delimiter = ''
            print(f'{delimiter} {matrix[i][j]:^{lens[j]}}  ', end='')
        print()
    print()
...
def print_matrix(matrix):
    """
    Prints a matrix with aligned spacing along the length of the maximum element.
...
def print_matrix2(matrix):
    """
    Prints a matrix with aligned column spacing.
